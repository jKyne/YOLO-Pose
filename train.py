...
def train(epoch):
    ...
    # TRAINING
    lr = adjust_learning_rate(optimizer_2, processed_batches)
    ...
    for batch_idx, (data, target, gt_distance, gt_rotation) in enumerate(train_loader):
        t2 = time.time()
        torch.cuda.empty_cache()
        # adjust learning rate
        adjust_learning_rate(optimizer_3, processed_batches)
        ...
        all_boxes = get_region_boxes(output.data, conf_thresh, num_classes) 
        all_boxes = np.reshape(np.array(all_boxes),(-1,15))
        all_boxes = torch.from_numpy(all_boxes).float()
        if use_cuda:
            all_boxes = all_boxes.cuda()
        model.seen = model.seen + data.data.size(0)
        region_loss.seen = region_loss.seen + data.data.size(0)
        # Compute loss of bounding box, grow an array of losses for saving later on
        #loss_box = region_loss(output, target)
        
        training_iters.append(epoch * math.ceil(len(train_loader.dataset) / float(batch_size) ) + niter)
        niter += 1
        t7 = time.time()
        # Compute loss of translation matrix
        output_distance = model_2(all_boxes).float()
        if use_cuda:
            gt_distance = gt_distance.cuda()
            gt_rotation = gt_rotation.cuda()

        loss_distance = distance_loss(output_distance, gt_distance)
        #compute of rotation matrix
        input_final = torch.cat((output_distance, all_boxes),1)
        output_rotation = model_3(input_final)
        loss_rotation = rotation_loss(output_rotation, output_distance, gt_rotation, gt_distance)

        # Backprop: compute gradient of the loss with respect to model parameters
        #loss = loss_box + 5*loss_distance + 10*loss_rotation
        loss =  loss_distance + 2*loss_rotation
        print("epoch:%d, batch: %d, loss_T:%f, loss_R:%f" %(epoch,batch_idx,loss_distance,loss_rotation))
        loss.backward()
        ...
        for i in range(output.size(0)):
            ...
            for k in range(num_gts):
                ...
                for j in range(len(boxes)):
                    if boxes[j][12] > best_conf_est:
                        best_conf_est = boxes[j][12]
                        box_pr        = boxes[j]
                        match         = corner_confidence9(box_gt[:12], torch.FloatTensor(boxes[j][:12]))

                # Denormalize the corner predictions 
                corners2D_gt = np.array(np.reshape(box_gt[:12], [6, 2]), dtype='float32')
                corners2D_pr = np.array(np.reshape(box_pr[:12], [6, 2]), dtype='float32')
...
if __name__ == "__main__":
    ...
    # Specify the model and the loss of target detection
    model       = Darknet(cfgfile)
    region_loss = model.loss
    # Specify the model and the loss of pose estimation
    model_2     = DistanceNet()
    distance_loss = model_2.loss
    model_3     = RotationNet()
    rotation_loss = model_3.loss
    ...
    if use_cuda:
        ...
        #model = torch.nn.DataParallel(model, device_ids=[0,1,2,3]).cuda() # Multiple GPU parallelism
    ...
    #optimizer_1 = optim.Adam(model.parameters(), lr=0.001) # Adam optimization
    ...
    if evaluate:
        ...
        for epoch in range(init_epoch, max_epochs): 
            ...
            if (epoch % 20 == 0) and (epoch is not 0): 
                #model.save_weights('%s/model_%s.weights' % (backupdir,epoch))
                print(1)
                torch.save(model_2.state_dict(), '%s/model_distance_%s.pkl' % (backupdir,epoch))
                print(2)
                torch.save(model_3.state_dict(), '%s/rotation_%s.pkl' % (backupdir,epoch))
                print(3)
