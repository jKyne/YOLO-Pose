...
def fill_truth_detection(labpath,label_pose_path, w, h, flip, dx, dy, sx, sy):
    max_boxes = 50
    label = np.zeros((max_boxes,15))
    label_1 = np.zeros((1,3))
    label_2 = np.zeros((1,4))
    if os.path.getsize(labpath):
        bs = np.loadtxt(labpath)
        if bs is None:
            return label, label_1, label_2
        bs = np.reshape(bs, (-1, 15))
        cc = 0
    ...
    if os.path.getsize(label_pose_path):
        ds = np.loadtxt(label_pose_path)
        if ds is None:
            return label, label_1, label_2
        ds = np.reshape(ds, (-1, 7))
        cc = 0
    ...
    label_1 = np.reshape(label_1, (-1))
    label_2 = np.reshape(label_2, (-1))
...
def load_data_detection(imgpath, shape, jitter, hue, saturation, exposure):
    label_pose_path = imgpath.replace('images', 'labels_pose').replace('JPEGImages', 'labels_pose').replace('.jpeg', '.txt').replace('.jpg','.txt')
    labpath = imgpath.replace('images', 'labels').replace('JPEGImages', 'labels').replace('.jpeg', '.txt').replace('.png','.txt')
    ...
    ow, oh = img.size
    label,label_1,label_2 = fill_truth_detection(labpath, label_pose_path, ow, oh, flip, dx, dy, 1./sx, 1./sy)
